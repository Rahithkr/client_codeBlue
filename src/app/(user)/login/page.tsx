...
useEffect(()=>{
  ...
  const [formData, setFormData] = useState<formData>({
    ...
    const navigateForgotPassword=(e:any)=>{
        e.preventDefault()
    ...
    const handleSubmit=async(e:React.FormEvent<HTMLFormElement>)=>{
     ...
     try{
        ...
        localStorage.setItem('codeBlueUser-token', data.token);
        document.cookie=`codeBlueUser-token=${data.token};path=/;`
        router.push(`/home?email=${formData.email}`)
      }catch(error:any){
  ...
  return (
    ...
    alt="Ambulance staff"
    width={800}  // Adjust width and height as per your design requirements
    height={600}
  ...
  />    </div>
    ...
    <div className="flex justify-center items-center w-full md:w-1/2 p-5">
      <form className="max-w-sm w-full" onSubmit={handleSubmit}>
        <h1 className="text-4xl mb-8 text-gray-900 dark:text-white"> User Login</h1>
        <div className="mb-5">
        ...
        <button onClick={navigateForgotPassword}><a href="" className="text-sm text-gray-900 dark:text-white mt-2 ms-8">Forgot password?</a></button>
        </div>
        
        {errorMessage && <p className="text-red-500">{errorMessage}</p>}
        <p className='text text-slate-900 ms-8 mt-3' >Don&apos;t have an account ?<a href="/signup"><span className='text text-blue-700'> Sign up </span></a></p>
